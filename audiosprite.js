...
module.exports = function(files) {
    ...
    function exportFileCaf(src, dest, cb) {
        ...
        else spawn('afconvert', ['-f', 'caff', '-d', 'ima4', src, dest])
            .on('exit', function(code, signal) {
                if (code) {
                    return cb({
                        msg: 'Error exporting file',
    ...
    function processFiles() {
        ...
        async.forEachSeries(files, function(file, cb) {
            ...
            makeRawAudioFile(file, function(err, tmp) {
                ...
                var name = file.replace(/\.[a-zA-Z0-9]+$/, '');
            ...
            async.forEachSeries(Object.keys(formats), function(ext, cb) {
                ...
                case 'simple':
                    finalJson.file = path.basename(json.resources[0],
                                                   path.extname(json.resources[0]));
                    finalJson.sprites = {};
                    for (sn in json.spritemap) {
                        spriteInfo = json.spritemap[sn];
                        finalJson.sprites[sn] = {
                            start: spriteInfo.start,
                            length: spriteInfo.end - spriteInfo.start
                        };
                    }
                    break;
